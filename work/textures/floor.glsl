...
void wood_v1(vec2 uv, vec2 seed, out vec3 albedo, out float height)
{
    vec2 branch_id;
    uv += random_vec2(seed);
    vec2 branch_uv = uv_tilling(uv, branch_id, vec2(5.0));
...
vec3 v4(vec2 uv, vec2 seed)
{
    ...
    wood_v1(wood_uv * 0.1, plank_seed++, wood_albedo, wood_height);
...
void wood_v2(vec2 uv, vec2 seed, out vec3 albedo, out float height)
{
    vec2 branch_id;
    uv += random_vec2(seed);
    vec2 branch_uv = uv_tilling(uv, branch_id, vec2(5.0));
    ...
    branch_distortion *= step(random(branch_seed++), 0.5);
    float branch_area = step(0.8, branch_distortion);
    vec3 branch_albedo = mix(color_hex(0x40251C), color_hex(0x76584D), random(branch_seed++));
    ...
    albedo = mix(color_hex(0xEAC7A1), color_hex(0x5A311D), easing_power_in(rings, 3.0));
    albedo = mix(albedo, branch_albedo, branch_area);
}

vec3 v5(vec2 uv, vec2 seed)
{
    vec2 plank_id;
    vec2 plank_seed;
    vec2 plank_scale = vec2(20.0, 1.0);
